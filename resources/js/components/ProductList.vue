    ...
    <div class="col-12">
        <div class="product-header-wrap">
            ...
            <div class="show-product-area">
                <p class="show-product">Showing {{this.currentPage * this.recordsPerPage + 1}} - {{Math.min((this.currentPage + 1) * this.recordsPerPage, this.count)}} of {{this.count}} result</p>
            </div>
        </div>
                ...
                <div class="tab-pane fade active show" id="nav-grid" role="tabpanel" aria-labelledby="nav-grid-tab">
                    <div class="row">
                        <product v-bind:picUrl="product.picUrl"
                                 ...
                                 v-bind:companyId="product.companyId"
                                 v-bind:itemId="product.itemId"
                                 v-for="product in this.PRODUCTS.slice(this.currentPage * this.recordsPerPage, this.currentPage * this.recordsPerPage + this.recordsPerPage)">
                        </product>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <div class="pagination-content-wrap">
                                <nav class="pagination-nav">
                                    <ul class="pagination justify-content-center">
                                        <li>
                                            <a class="prev" v-bind:class="{disabled: this.currentPage === 0}" @click="loadPrevPage">
                                        ...
                                        <li v-for="page in this.pages">
                                            <a
                                                v-bind:class="{disabled: (page === currentPage + 1 && page === 1) || page === currentPage}"
                                                @click="setPage(page)">
                                                {{page}}
                                            </a>
                                        </li>
                                        <li>
                                            <a class="next" v-bind:class="{disabled: this.currentPage === this.pages[this.pages.length - 1]}" @click="loadNextPage">Next
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="20" height="20">
    ...
    computed: {
        ...
        count() {
            ...
            if (this.PRODUCTS === null)
                return 0;
            for (let i = 0; i < this.PRODUCTS.length - this.recordsPerPage; i += this.recordsPerPage) {
                this.pages.push(i / this.recordsPerPage + 1);
            }
